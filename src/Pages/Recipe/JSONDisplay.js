...
const JSONDisplay = () => {
    ...
    console.log('현재 선택된 block id:', selectedBlockId)

    const filteredBlockData = blockData.filter(item => {
        // rcp_id가 일치하지 않으면 무조건 제외
        if (item.rcp_id !== "rcp_sim_250818_108") {
            return false;
        }
        // selectedBlockId가 없거나(null, undefined, 빈 문자열) 일치하면 포함
        if (!selectedBlockId || item.block_id === selectedBlockId) {
            return true;
        }
        return false;
    });
